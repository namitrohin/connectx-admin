...
function AddOPSManager() {
  ...
  const handlePdfUpload = async (files) => {
    ...
    try {
      await userService
        .uploadImage('/api/v0/app/opsManager', data)
        .then((result) => {
  ...
  //   },[formData.branch_id])
  return (
    <MainLayout pageName="Add OPS Manager" hasAddButton={false}>
      <Container className="formwrapper mt-0" fluid>
        <div className="d-flex justify-content-end w-100 mb-3">
          <Link className="backlink" to="/users/ops-manager">
          ...
          <Row>
            <Col md={12}>
              <Row>
                ...
                <Col md={4}>
                  {formData.p_branch_id && (
                    <AutoCompletedDropdown
                      url={"/api/v0/web/web_branch_manager_dropdown"}
                      body={{
                        p_search: "",
                        p_limit: 10,
                        branch_id: formData.p_branch_id,
                      }}
                      handleDataChange={(val) => {
                        setFormData({
                          ...formData,
                          user_id: val.branch_id,
                          p_branch_manager_id: val.branch_id,
                          emp_code: val.emp_code,
                        });
                      }}
                      valueInput={formData.emp_code}
                      objLevel={"emp_code"}
